...
package com.jeremy.common.service;

import com.google.common.collect.Lists;
import com.jeremy.common.persistence.BaseEntity;
import com.jeremy.common.utils.SpringContextHolder;
import com.jeremy.common.utils.StringUtils;
import com.jeremy.modules.sys.dao.RoleDao;
...
//							}
     ...
     */
    public static String dataScopeFilter(BaseEntity<?> entity, String officeWheres, String userWheres, String permission) {
        ...
        // 获取用户当前菜单的最大的数据权限范围
        Role role = SpringContextHolder.getBean(RoleDao.class).getDataScope(user.getId(), permission);
        String dataScopeString = "7";
        String roleId = "";
        if (null != role) {
            dataScopeString = role.getDataScope();
            roleId = role.getId();
        }
        ...
        for (String where : StringUtils.split(officeWheres, ",")) {
            if (Role.DATA_SCOPE_COMPANY_AND_CHILD.equals(dataScopeString)) {
                // 包括本公司下的部门 （type=1:公司；type=2：部门）
                sqlString.append(" EXISTS (SELECT 1 FROM sys_office");
                ...
                where = where.replace("office_id", "company_id");
                ...
                sqlString.append(" EXISTS (SELECT 1 FROM sys_role_office ro123456, sys_office o123456");
        ...
        for (String where : StringUtils.split(userWheres, ",")) {
            if (Role.DATA_SCOPE_SELF.equals(dataScopeString)) {
                sqlString.append(" EXISTS (SELECT 1 FROM sys_user");
